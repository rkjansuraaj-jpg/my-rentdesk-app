...
export default function Reports() {
  ...
  const report = useMemo(() => {
    ...
    const longRenters = activeRows
      .map((r) => {
        const days = Math.round((TODAY - new Date(r.item.segments[0].from)) / (1000 * 60 * 60 * 24));
        return { order: r.order, item: r.item, customer: customers.find((c) => c.id === r.order.customerId), days };
      })
      .filter((r) => r.days > 3);
      ...
      <div style={{ maxWidth: 700 }}>
        ...
        <div style={{ background: COLORS.CARD, border: `1px solid ${COLORS.LINE}`, borderRadius: 12, overflow: "hidden" }}>
          ...
          {report.longRenters.map((r, i) => (
            <div key={r.item.itemId} style={{ display: "flex", justifyContent: "space-between", padding: "13px 18px", borderTop: i === 0 ? "none" : `1px solid ${COLORS.LINE}` }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, color: COLORS.INK }}>{r.customer?.name}</div>
                <div style={{ fontSize: 12, color: COLORS.INK_SOFT }}>{r.item.machine}</div>
              </div>
